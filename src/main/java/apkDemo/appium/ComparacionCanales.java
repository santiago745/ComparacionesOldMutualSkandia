...
public class ComparacionCanales {
	UtilsMobile ut;
	PrintWriter out;
	PrintWriter out2;
	...
	public void teardown() {
	 ...
	 public void revisarPaginaMobile() {
    	 ...
    	 try {
    		 //src/main/java/
 			out = new PrintWriter("recursos/domTempCO.txt");
 			String domCO=ut.driver.getPageSource();
 			domCO=ut.prettyprintxml(domCO);
 			out.println(domCO);
 			File f = new File("recursos/domCO.txt");
 			if(!f.exists()) { 
 			    out2 = new PrintWriter("recursos/domCO.txt");
 				out2.println(domCO);
 				out2.close();
 			}
    	 ...
    	 List<String> lista =compareDoms("recursos/domCO.txt","recursos/domTempCO.txt");
		for(int i=0;i<lista.size();i++) {
			
			System.out.println(lista.get(i));
		}
		revisarLookAndFeel();
		compareImages(ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookCO.png", ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookTempCO.png");
    }
	 
	public void revisarLookAndFeel(){
		ut.screenshot("DOMLookTempCO");
		File f = new File("recursos/DOMLookCO.png");
		if(!f.exists()) { 
			ut.screenshot("DOMLookCO");
			File scrFile = ((TakesScreenshot)ut.driver).getScreenshotAs(OutputType.FILE);
			try {
				FileUtils.copyFile(scrFile, new File("recursos/DOMLookCO.png"));
			} catch (IOException e) {
				System.err.println("No se pudo tomar el pantallazo "+e.getMessage());
			}
		}else {
			try {
				Files.copy(Paths.get("recursos/DOMLookCO.png"), Paths.get(ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookCO.png"), StandardCopyOption.REPLACE_EXISTING);
			} catch (IOException e) {
