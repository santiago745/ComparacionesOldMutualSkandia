...
public class ComparacionCertificados {
	UtilsMobile ut;
	PrintWriter out;
	PrintWriter out2;
	...
	public void teardown() {
	 ...
	 public void revisarPaginaMobile() {
    	 ...
    	 try {
    		 //src/main/java/
 			out = new PrintWriter("recursos/domTempCC.txt");
 			String domCC=ut.driver.getPageSource();
 			domCC=ut.prettyprintxml(domCC);
 			out.println(domCC);
 			File f = new File("recursos/domCC.txt");
 			if(!f.exists()) { 
 			    out2 = new PrintWriter("recursos/domCC.txt");
 				out2.println(domCC);
 				out2.close();
 			}
    	 ...
    	 List<String> lista =compareDoms("recursos/domCC.txt","recursos/domTempCC.txt");
		for(int i=0;i<lista.size();i++) {
			
			System.out.println(lista.get(i));
		}
		revisarLookAndFeel();
		compareImages(ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookCC.png", ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookTempCC.png");
    }
	 
	public void revisarLookAndFeel(){
		ut.screenshot("DOMLookTempCC");
		File f = new File("recursos/DOMLookCC.png");
		if(!f.exists()) { 
			ut.screenshot("DOMLookCC");
			File scrFile = ((TakesScreenshot)ut.driver).getScreenshotAs(OutputType.FILE);
			try {
				FileUtils.copyFile(scrFile, new File("recursos/DOMLookCC.png"));
			} catch (IOException e) {
				System.err.println("No se pudo tomar el pantallazo "+e.getMessage());
			}
		}else {
			try {
				Files.copy(Paths.get("recursos/DOMLookCC.png"), Paths.get(ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookCC.png"), StandardCopyOption.REPLACE_EXISTING);
			} catch (IOException e) {
