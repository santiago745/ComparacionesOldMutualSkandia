...
public class ComparacionLogin {
	...
	public void teardown() {
	 ...
	 public void revisarPaginaMobile() {
    	 ...
    	 try {
    		 //src/main/java/
 			out = new PrintWriter("recursos/domTempCL.txt");
 			String domCL=ut.driver.getPageSource();
 			domCL=ut.prettyprintxml(domCL);
 			out.println(domCL);
 			File f = new File("recursos/domCL.txt");
 			if(!f.exists()) { 
 			    out2 = new PrintWriter("recursos/domCL.txt");
 				out2.println(domCL);
 				out2.close();
 			}
    	 ...
    	 List<String> lista =compareDoms("recursos/domCL.txt","recursos/domTempCL.txt");
		for(int i=0;i<lista.size();i++) {
			
			System.out.println(lista.get(i));
		}
		revisarLookAndFeel();
		compareImages(ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookCL.png", ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookTempCL.png");
    }
	 
	public void revisarLookAndFeel(){
		ut.screenshot("DOMLookTempCL");
		File f = new File("recursos/DOMLookCL.png");
		if(!f.exists()) { 
			ut.screenshot("DOMLookCL");
			File scrFile = ((TakesScreenshot)ut.driver).getScreenshotAs(OutputType.FILE);
			try {
				FileUtils.copyFile(scrFile, new File("recursos/DOMLookCL.png"));
			} catch (IOException e) {
				System.err.println("No se pudo tomar el pantallazo "+e.getMessage());
			}
		}else {
			try {
				Files.copy(Paths.get("recursos/DOMLookCL.png"), Paths.get(ut.screenshotFolder+"/"+ut.fecha+"/screenshots/DOMLookCL.png"), StandardCopyOption.REPLACE_EXISTING);
			} catch (IOException e) {
